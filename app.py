...
def check_email_validity(email):
    ...
    if domain in fake_domains:
        return False, f"⚠️ Domain `{domain}` is known for temporary/fake emails"
...
if st.button("Scan Email"):
    ...
    if is_phish:
        st.error(f"⚠️ This email may be phishing! ({len(matched_keywords)} suspicious keywords found)")
